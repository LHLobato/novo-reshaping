...
for state in states:
    ...
    X_text_train, X_text_temp, y_train, y_temp = train_test_split(
        payload, labels, test_size=0.3, random_state=state, stratify=labels
    )
    ...
    joblib.dump(selector, f"selector{state}-gray-http.joblib")

    #joblib.dump(pca, f"pca{state}.joblib")
    joblib.dump(vectorizer, f"tfidf2{state}-gray-http.joblib")
    ...
    for image_type, transformer in image_reshapes.items():
        ...
        base_dir = f"../../images/HTTP-PARAMS/{image_type}_state{state}" # Diretório específico para o state
