...
def get_args():
    ...
    parser.add_argument(
        "--dataset", type=str, required=True, help="Nome do dataset (ex: CSIC-2010)"
    )
    parser.add_argument(
        "--image_dir",
        type=str,
...
def _already_evaluated(
    results_csv: str, model_name: str, dataset_name: str, clf_name: str
) -> bool:
...
def main(args):
    ...
    if dataset_cfg is None:
        raise ValueError(
            f"type_img {args.type_img!r} não encontrado em config.get_datasets."
        )

    print(f"\n{'=' * 60}")
    print(
        f"Modelo: {args.model_name} | Dataset: {args.dataset} | Tipo: {args.type_img}"
    )
    print(f"Checkpoint: {ckpt_path}")
    print(f"{'=' * 60}")

    feat_val_path = os.path.join(
        args.features_dir, f"val-{args.model_name}-{args.dataset}-{args.type_img}.npz"
    )
    feat_test_path = os.path.join(
        args.features_dir, f"test-{args.model_name}-{args.dataset}-{args.type_img}.npz"
    )
    ...
    for clf_name, clf_factory in registry.items():
        if _already_evaluated(
            args.results_csv, args.model_name, args.dataset, clf_name
        ):
