...
def train(
    ...
    if cond3:
        print(
            "!!! CoAtNet/MaxViT detectado: DESLIGANDO Mixed Precision (AMP) para estabilidade !!!"
        )
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        try:
            ...
            print(
                f"Erro ao calcular AUC (provavelmente apenas 1 classe detectada): {e}"
            )
            train_auc = 0.5
        ...
        print(
            f"| Train Loss: {running_loss / len(train_loader):.4f} | Train Acc: {train_accuracy:.4f} | ROC-AUC: {train_auc:.4f}"
        )
        print(
            f"Val Loss: {val_loss / len(val_loader):.4f} | Val Acc: {val_accuracy:.4f} | ROC-AUC: {val_auc:.4f}"
        )
...
def test(model, test_loader, model_name, device="cuda"):
    ...
    if cond3:
        print(
            "!!! Teste: CoAtNet/MaxViT detectado. Desligando AMP (FP32) para evitar NaN. !!!"
        )
        use_amp = False
    ...
    if len(test_labels) > 0:
        ...
        try:
            ...
            print(
                "Erro ao calcular ROC-AUC no teste (provável NaN residual ou classe única). Definindo como 0.5"
            )
            test_auc = 0.5
    else:
